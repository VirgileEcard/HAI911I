...
public:
    ...
    void buildCotangentWeightsOfTriangleMesh( const Mesh& mesh)
    {
        ...
        for( unsigned int t = 0 ; t < mesh.T.size() ; ++t )
        {
            ...
            Vec3 p0( mesh.V[v0][0] , mesh.V[v0][1] , mesh.V[v0][2] );
            Vec3 p1( mesh.V[v1][0] , mesh.V[v1][1] , mesh.V[v1][2] );
            Vec3 p2( mesh.V[v2][0] , mesh.V[v2][1] , mesh.V[v2][2] );

            //Arète p0-p1
            //on doit avoir l'angle en p2 -> on a besoin des deux arètes p2-p0 et p2-p1
            Vec3 edge1 = (p0 - p2);
            Vec3 edge2 = (p1 - p2);

            float denom = Vec3::cross(edge1, edge2).length();
            float cotangent = Vec3::dot(edge1, edge2) / denom;
            //cotangent = std::max(cotangent, 0.001f);

            edge_weights[v0][v1] += cotangent/2.f;
            edge_weights[v1][v0] += cotangent/2.f;

            //Arète p1-p2
            //on doit avoir l'angle en p0
            edge1 = (p1 - p0);
            edge2 = (p2 - p0);

            denom = Vec3::cross(edge1, edge2).length();
            cotangent = Vec3::dot(edge1, edge2) / denom;
            //cotangent = std::max(cotangent, 0.001f);
            
            edge_weights[v1][v2] += cotangent/2.f;
            edge_weights[v2][v1] += cotangent/2.f;

            //Arète p2-p0
            //on doit avoir l'angle en p1
            edge1 = (p0 - p1);
            edge2 = (p2 - p1);
            ...
            edge_weights[v0][v2] += cotangent/2.f;
            edge_weights[v2][v0] += cotangent/2.f;
        }
