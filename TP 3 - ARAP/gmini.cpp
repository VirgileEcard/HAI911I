...
void testlinearSystem() {
    // You can get inspiration from this piece of code :
    {
        ...
        std::cout <<"x0 = "<< X[0] << "  x2 = " << X[1] << "  x3 = " << X[2] << std::endl;
    }
}
//------------------------------------------------------------------------------------------------------//
//---------------------------------  EXAMPLE OF USE OF A LINEAR SYSTEM  --------------------------------//
...
void updateSystem() {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {        
        for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
            ...
            arapLinearSystem.A(3*equationIndex+0, 3*v+0) = -it->second;//-1;
            arapLinearSystem.A(3*equationIndex+1, 3*v+1) = -it->second;//-1;
            arapLinearSystem.A(3*equationIndex+2, 3*v+2) = -it->second;//-1;

            arapLinearSystem.A(3*equationIndex+0, 3*vNeighbor+0) = it->second;//1;
            arapLinearSystem.A(3*equationIndex+1, 3*vNeighbor+1) = it->second;//1;
            arapLinearSystem.A(3*equationIndex+2, 3*vNeighbor+2) = it->second;//1;
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        if(verticesHandles[v] != -1) {
            ...
            arapLinearSystem.A(3*equationIndex + 3*handleIndex+0, 3*v+0) = 1;
            arapLinearSystem.A(3*equationIndex + 3*handleIndex+1, 3*v+1) = 1;
            arapLinearSystem.A(3*equationIndex + 3*handleIndex+2, 3*v+2) = 1;
    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
                ...
                arapLinearSystem.b(3*equationIndex + 0) = rotatedEdge(0);
                arapLinearSystem.b(3*equationIndex + 1) = rotatedEdge(1);
                arapLinearSystem.b(3*equationIndex + 2) = rotatedEdge(2);
                equationIndex++;
            }
        }
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            if(verticesHandles[v] != -1) {
                ...
                // WHAT TO PUT HERE ??????? How to update the entries of b ?
                arapLinearSystem.b(3*equationIndex + 3*handleIndex+0) = mesh.V[v].p[0];
                arapLinearSystem.b(3*equationIndex + 3*handleIndex+1) = mesh.V[v].p[1];
                arapLinearSystem.b(3*equationIndex + 3*handleIndex+2) = mesh.V[v].p[2];
                handleIndex++;
            }
        }
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            ...
            for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
                ...
                // cf p.22 encadré bleu 
                // Rédac du prof :
                // AX = b =>e_ij
                // T = SOMME(e_ij' e_ij^t)   (matrice)  ->cf déf du sommet dans MeshVertex pour voir comment ça marche
                // (U, V) = SVD(T)              \
                // R = U.diag(1,1,det(U,V)).V   /  GetClosestRotation()
                tensorMatrix +=(it->second) * rotatedEdge * initialEdge.transpose();
                //tensorMatrix +=(it->second) * initialEdge * rotatedEdge.transpose();
...
int main (int argc, char ** argv) {
    ...
    /***DEBUG***/
    for(unsigned int v = 0; v < std::min(3u, (unsigned int)mesh.V.size()); ++v) {
